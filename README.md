...
Данное приложение является учебным и предназначено для получения актуальной статистики по заболеваемости Covid-19 по странам. 
...
[6. Примечания](#notes)  
[7. Приложение на разных устройствах](#screens)
...
* Поделиться - выполняется скриншот экрана с данными статистки, который доступен для сохранения или отправки
...
На данном экране отображается информация о симптомах болезни, а так же рекомендации по противодействию распространения вируса. На скриншоте №2 демонстрируется наличие scrollView.
...
В качестве архитектуры используется _SOA + MVC_. Приложение разделено на 4 слоя:
...
* AssemblyBuilder - главный сборщик приложения, пока в текущей реализации собирает только контроллеры.
* MainRouter - сервис, который управляет навигацией в приложении. Открытие или закрытие экранов происходит с его помощью.
* NetworkingService - сервис работы с сетью, которые загружает данные по статистике и изображение флага страны. Для работы использует компонент с уровня Core «RequestSender», которые непосредственно отправляется в сеть и возвращается с данными. Данный компонент требует конфигурацию, которая в свою очередь содержит сам запрос (с нужным URL) и парсер, которые обрабатывает полученные данные из сети. Для удобства и масштабирования создана фабрика запросов «RequestsFactory».
...
* Обработка ошибки загрузки изображения так же сознательно не обработана, так как в контексте приложения не является главным элементом работы. Возможно, правильнее такой случай обработать и формировать данные для сбора аналитики по ошибкам, но данная функциональность в приложении не реализована.
...
* Сознательно не комментировал дополнительно код (кроме Mark), так как придерживаюсь правилу «Код должен сам говорить за себя». Плюс, проект небольшой и не супер сложный, поэтому дополнительное комментирование излишне. А если непонятно, как работает - значит плохой нейминг, а это уже другая история.
* Как описано выше, согласно требованиям Google, допустимая длин строки 100 символов. Файл swiftlint.yml от Сбера, которым нам скинули, содержит ограничение в 120 символов. В связи с тем, что значения различаются, я допустил, что можно использовать свое значение и увеличил его до 140 символов. И это только по той причине, что это свой проект. На существующих естественно я бы вписывался в 120 или 100 символов, в зависимости что будет указано в регламенте к разработке.
* По умолчанию, при первом запуске приложения в качестве стрын подставляется «Россия». Геолокацию запилить не успел, сделал не совсем красиво.

<a name="notes"><h2>Примечания</h2></a>
Дизайн был выполнен на основе существующего приложения. На это есть две причины: я очень плохой дизайнер и частичный дальтоник. То что красиво для меня, для других может оказаться достаточно странно.

Если вдруг при сборке приложения появится ошибка _«Command CompileAssetCatalog failed with a nonzero exit code»_, то достаточно сделать Clean build и запустить приложение. Я скачивал и запускал проект на другой машине, там все было окей. У меня данная ошибка уже не возникает, но до этого были с этим проблемы. 
Да, я каюсь за это, но все найденные/нагугленные решения данной проблемы сводились к сделать чистую сборку, перезапуск, С чем это связано, я так и не понял и надеюсь, что при проверке данная ошибки не возникнет и проблема локальная (только у меня из кэшей или чего-то другого).

Проект выполнял, разбивая его на подзадачи, вел в Trello. Всего выполнено 44 задачи, отказ от 11 задач (причины: не успею, не рационально и пр.).

<a name="screens"><h2>Приложение на разных устройствах</h2></a>
