...
Если вдруг при сборке приложения появится ошибка _«Command CompileAssetCatalog failed with a nonzero exit code»_, то достаточно сделать Clean build и запустить приложение. 

Я скачивал и запускал проект на другой машине, там все было окей. У меня данная ошибка уже не возникает, но до этого были с этим проблемы. 
